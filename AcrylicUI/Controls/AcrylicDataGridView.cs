...
{
    public class AcrylicDataGridView : UserControl, ISupportInitialize
    {
        ...
        private void BaseKeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (Clipboard.GetDataObject()?.GetData(DataFormats.UnicodeText) is not string clipboardString)
                    return;
        ...
        private void BaseDragOver(object sender, DragEventArgs e)
        {
            ...
            DragPosition = info.Type switch
            {
                DataGridViewHitTestType.Cell or DataGridViewHitTestType.RowHeader => info.RowIndex,
                DataGridViewHitTestType.None => Rows.Count,
                _ => -1,
            };
        }
        ...
        {
            // Get drag data
            if (e.Data.GetData(typeof(DragDropMetaData)) is not DragDropMetaData metaData || DragPosition < 0)
                return;
            ...
            object[] followingRows = Rows.Cast<DataGridViewRow>()
                .Where(row => row.Index >= DragPosition && row.Index < rows.Count)
                .Except(SelectedRows.Cast<DataGridViewRow>())
                .Select(row => rows[row.Index])
                .ToArray();
        ...
        private void BaseCellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            // Invoke event
            var args = new AcrylicDataGridViewSafeCellFormattingEventArgs(e.ColumnIndex, e.RowIndex, e.Value, e.DesiredType, e.CellStyle, column, row);
        ...
        public event AcrylicDataGridViewSafeCellFormattingEventHandler CellFormattingSafe;
    ...
    public class AcrylicDataGridViewButtonCell : DataGridViewButtonCell
    {
        ...
        public bool Enabled
        {
            get
            {
                return _enabled ?? (OwningColumn as AcrylicDataGridViewButtonColumn)?.Enabled ?? true;
            }
            set
            {
                if (value == Enabled)
    ...
    public class AcrylicDataGridViewButtonColumn : DataGridViewButtonColumn
    {
        ...
        public AcrylicDataGridViewButtonColumn()
        {
            CellTemplate = new AcrylicDataGridViewButtonCell();
    ...
    public class AcrylicDataGridViewCheckBoxCell : DataGridViewCheckBoxCell
    { }

    public class AcrylicDataGridViewCheckBoxColumn : DataGridViewCheckBoxColumn
    {
        public AcrylicDataGridViewCheckBoxColumn()
        {
            base.FlatStyle = FlatStyle.Flat;
        }
    ...
    public class AcrylicDataGridViewSafeCellFormattingEventArgs : DataGridViewCellFormattingEventArgs
    {
        public DataGridViewColumn Column { get; }
        public DataGridViewRow Row { get; }

        public AcrylicDataGridViewSafeCellFormattingEventArgs(int columnIndex, int rowIndex, object value,
